...
public class TreeManager {
    ...
    public PathTreeNode createBranchAndInsertFromRoot (JTree tree, PathTreeNode root, PathComparator comparator, boolean compFlag)
    {
        if (threadToWatch == null) {
            threadToWatch = new runnableProcessWatch();
            threadToWatch.start();
        }
        comparator.setError(false);
        if (root == null) {
            comparator.setError();
            ...
            for (Path child : stream) {
                ...
                if (file.exists() == true) {
                    ...
                    if (file.isDirectory() == true) {
                        ...
                        if (compFlag == true) {
                            childNode.setMatch(comparator.compareAndCollect(file, true));
                        }
                        if (file.isHidden() == true) {
    ...
    public PathTreeNode createBranchAndInsertFromSelected (JTree tree, PathComparator comparator, boolean compFlag)
    {
        return createBranchAndInsertFromRoot(tree, (PathTreeNode) tree.getLastSelectedPathComponent(), comparator, compFlag);
    }

    private void recursiveScan (JTree tree, PathTreeNode node, PathComparator comparator, boolean watchAll, boolean hide)
    {
        int items = 0;
        if (node == null) {
            return;
        ...
        if (node.isLeaf() == true) {
            File file = ((Path) node.getUserObject()).toFile();
            if (file.isDirectory()) {
                createBranchAndInsertFromRoot(tree, node, comparator, true);
            } else {
                return;
            }
        }
        ...
        for (int i = 0; i < items; i++) {
            recursiveScan(tree, (PathTreeNode)node.getChildAt(i), comparator, watchAll, hide);
        }
    }

    public String watchAllLeafsInTree (JTree tree, PathComparator comparator, boolean watchAll, boolean hide)
    {
        DefaultTreeModel model = (DefaultTreeModel)tree.getModel();
        ...
        if (root.isLeaf() == true) {
            this.createBranchAndInsertFromRoot(tree, root, comparator, watchAll);
        }
        this.recursiveScan(tree, root, comparator, watchAll, hide);
